...
void main() {
  ...
  testWidgets(
    'ride document creation and scheduled ride persistence work',
    (tester) async {
  ...
  testWidgets(
    'ride status updates are persisted through completion',
    (tester) async {
      final rider = await createRoleFixture(
        role: UserRole.rider,
  ...
  testWidgets(
    'authenticated users can access only allowed Firestore data',
    (tester) async {
      final driver = await createRoleFixture(
        role: UserRole.driver,
